...
if ( isset( $_POST['active_user'] ) ) {
    // phpcs:disable
    $act_user_id = isset( $_POST['act_user_id'] ) ? intval( wp_unslash( $_POST['act_user_id'] ) ) : 0;
    $class      = get_user_meta( $act_user_id, 'class_name', true );
    $roll_id_in = isset( $_POST['roll_id'] ) ? sanitize_text_field( wp_unslash( $_POST['roll_id'] ) ) : '';
    $args = array(
        'meta_query' => array(
            'relation' => 'AND',
            array(
...
if ( isset( $_GET['action'] ) && $action === 'deactivate' ) {
    if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'deactive_action' ) ) {
        $student_id_raw = isset( $_GET['student_id'] ) ? sanitize_text_field( wp_unslash( $_GET['student_id'] ) ) : '';
...
if ( isset( $_POST['approve_comment'] ) ) {
    ...
    $student_id_param = isset( $_GET['student_id'] ) ? sanitize_text_field( wp_unslash( $_GET['student_id'] ) ) : '';
    if ( $result ) {
...
if ( isset( $_GET['action'] ) ) {
    $action_req = sanitize_key( wp_unslash( $_GET['action'] ) );
} else {
    $action_req = '';
}
if ( $action_req === 'delete' && isset( $_GET['leave_id'] ) ) {
    if ( isset( $_GET['_wpnonce_action'] ) && wp_verify_nonce( $_GET['_wpnonce_action'], 'delete_action' ) ) {
        $leave_id_raw = sanitize_text_field( wp_unslash( $_GET['leave_id'] ) );
        $leave_id     = intval( mjschool_decrypt_id( $leave_id_raw ) );
        $result       = $mjschool_obj_leave->mjschool_delete_leave( $leave_id );
        $student_id_action = isset( $_GET['student_id_action'] ) ? sanitize_text_field( wp_unslash( $_GET['student_id_action'] ) ) : '';
        if ( $result ) {
...
if ( isset( $_POST['exportstudentin_csv'] ) ) {
    ...
    if (! empty($student_list) ) {
        ...
        foreach ( $student_list as $retrive_data ) {
            ...
            $class_sections_name = $mjschool_class->mjschool_get_class_sections_name($class_section_id);
...
if ( isset( $_POST['save_student'] ) ) {
    ...
    if ( wp_verify_nonce( $nonce, 'save_teacher_admin_nonce' ) ) {
        ...
        $is_rollno = count( $userbyroll_no );
        if ( $action === 'edit' ) {
            $student_id = intval( mjschool_decrypt_id( sanitize_text_field( wp_unslash( $_POST['student_id'] ) ) ) );   
            $admission_no    = sanitize_text_field( wp_unslash( $_POST['admission_no'] ) ); 
            $args = array(
            ...
            if (isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'edit_action') ) {
                ...
                if ($roll_no_cheack === 1 ) {
                    ...
                    mjschool_append_audit_log('' . esc_html__('Student Updated', 'mjschool') . '( ' . $student . ' )' . '', $student_id, get_current_user_id(), 'edit', sanitize_text_field(wp_unslash($_POST['page'])));
        ...
        } else // ---------- Insert student. -----------//
        {
            ...
            if (! email_exists($_POST['email']) ) {
                if ($is_rollno ) {
                    ...
                    $student = $userdata['display_name'];
                    mjschool_append_audit_log('' . esc_html__('Student Added', 'mjschool') . '( ' . $student . ' )' . '', $result, get_current_user_id(), 'insert', sanitize_text_field(wp_unslash($_POST['page'])));
...
if ($action === 'delete' ) {
    if (isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], 'delete_action') ) {
        ...
        $result = $mjschool_obj_user->mjschool_delete_usedata(intval($student_id));
        if ($result ) {
...
// ---------- Student print. ------------//
if (isset($_GET['print']) && sanitize_text_field(wp_unslash($_GET['print'])) === 'pdf' ) {
    $sudent_id = intval(mjschool_decrypt_id(sanitize_text_field(wp_unslash($_GET['student']))));
}
// ------ Upload CSV Code. -----------//
            ...
            if (empty($errors) === true ) {
                ...
                foreach ( $rows as $row ) {
                    ...
                    if ($post_class_id === 0 ) {
                        ...
                        $class_id = $mjschool_class->mjschool_get_class_id_by_name($class_names);
                    } else {
                    ...
                    if (username_exists($username) ) { // If user exists, we take his ID by login.
                        ...
                        $studentExists    = false;
                        $admission_number = $mjschool_obj_admission->mjschool_generate_admission_number();
                    ...
                    foreach ( $fields as $field_name ) {
                        ...
                        if ($custom_field->field_type === 'date' ) {
                            $field_value = wp_date('Y-m-d', strtotime($csv[ $field_name ]));
                    ...
                    if ($user_created ) {
                        ...
                        if ( isset($_POST['mjschool_import_student_mail']) && sanitize_text_field(wp_unslash($_POST['mjschool_import_student_mail'])) === '1' ) {
                            if ($user_id ) {
        ...
        <div class="row"> <!------- Row Div. --------->
            <div class="col-md-12 mjschool-custom-padding-0">
                <div class="mjschool-main-list-page">
                    ...
                    if ( $active_tab === 'view_student_result' ) {
