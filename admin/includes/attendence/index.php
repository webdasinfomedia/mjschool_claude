					...
					if ( ! empty( $parent_list ) ) {
						...
						if ( isset( $_POST['mjschool_service_enable'] ) ) {
							foreach ( $parent_list as $user_id ) {
								$message_content = 'Your Child ' . mjschool_get_display_name( $stud->ID ) . ' is absent on ' . sanitize_text_field(wp_unslash($_POST['curr_date']));
								$type            = 'Attendance';
	...
	if ( ! empty( $student_attendance_list ) ) {
		...
		foreach ( $student_attendance_list as $retrive_data ) {
			if ( $retrive_data->role_name === 'student' ) {
				...
				$row[]   = $classname;
				$mjschool_class = new Mjschool_Class();
				$section = $mjschool_class->mjschool_get_section_name( $retrive_data->section_id );
				if ( ! empty( $section ) ) {
					$section = $section;
				} else {
					$section = '';
				...
				$subject = $mjschool_subject->mjschool_get_single_subject_name( $retrive_data->sub_id );
				if ( ! empty( $subject ) ) {
					$subject = $subject;
				} else {
					$subject = '';
	...
	if ( isset( $_FILES['csv_file'] ) ) {
		...
		if ( empty( $errors ) === true ) {
			...
			foreach ( $rows as $row ) {
				...
				$mjschool_class = new Mjschool_Class();
				$class_id = $mjschool_class->mjschool_get_class_id_by_name( $csv['class name'] );
				...
				$curr_date = wp_date( 'Y-m-d', strtotime( $csv['attendence_date'] ) );
...
if ( isset( $_POST['export_teacher_attendance_in_csv'] ) ) {
	...
	if ( empty( sanitize_text_field( wp_unslash( $_POST['filtered_date_type'] ) ) ) && empty( sanitize_text_field( wp_unslash( $_POST['filtered_member_id'] ) ) ) ) {
		...
		if ( ! empty( $_POST['filtered_member_id'] ) && sanitize_text_field(wp_unslash($_POST['filtered_member_id'])) !== 'all_teacher' ) {
			...
			$teacher_attendance_list = mjschool_get_member_attendence_beetween_start_date_to_end_date_for_admin( $start_date, $end_date, $member_id );
		} else {
			$type                    = 'teacher';
			$teacher_attendance_list = mjschool_get_all_student_attendence_beetween_satrt_date_to_enddate( $start_date, $end_date, $type );
		}
									...
									<div class="col-sm-3 col-md-3 col-lg-3 col-xl-3">
										<div class="form-group input">
											<div class="col-md-12 form-control">
												<input id="curr_date_for_index" class="form-control" type="text" value="<?php if ( isset( $_POST['curr_date'] ) ) { echo esc_attr( mjschool_get_date_in_input_box( sanitize_text_field(wp_unslash($_POST['curr_date'])) ) ); } else { echo esc_attr( mjschool_get_date_in_input_box( wp_date( 'Y-m-d' ) ) ); }?>" name="curr_date" readonly> 
						...
						<div class="form-body mjschool-user-form">
							<div class="row">
								<div class="col-sm-5 col-md-5 col-lg-5 col-xl-5">
									<div class="form-group input">
										<div class="col-md-12 form-control">
											<input id="curr_date_teacher" class="form-control" type="text" value="<?php if ( isset( $_POST['tcurr_date'] ) ) { echo esc_attr( mjschool_get_date_in_input_box( sanitize_text_field(wp_unslash($_POST['tcurr_date'])) ) ); } else { echo esc_attr( mjschool_get_date_in_input_box( wp_date( 'Y-m-d' ) ) ); } ?>" name="tcurr_date" readonly>
