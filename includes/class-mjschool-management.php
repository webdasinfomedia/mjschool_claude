...
class MJSchool_Management {
	...
	public function __construct( $user_id = null ) {
		if ( $user_id ) {
			...
			if ( $this->mjschool_get_current_user_role( $user_id ) === 'teacher' ) {
				...
				if ( isset( $data['own_data'] ) && $data['own_data'] === '1' ) {
	 ...
	 * @return string|false The user's role (lowercase) or false if not found.
	 */
	public function mjschool_get_current_user_role( $userid = 0 ) {
		if ( $userid != 0 ) {
			$current_user = get_userdata( intval( $userid ) );
		} else {
	 ...
	 * @param int $sid The ID of the section.
	 * @return array List of subjects.
	 */
	...
	private function mjschool_notice_board_student( $user_id, $role ) {
		...
		$sql = $wpdb->prepare(
			"SELECT * FROM $table_post AS post, $table_postmeta AS post_meta 
			WHERE post.post_type = 'notice' AND 
			(post.ID = post_meta.post_id AND (post_meta.meta_key = 'notice_for' AND 
			(post_meta.meta_value = %s OR post_meta.meta_value = 'all')) OR 
			(post_meta.meta_key = 'notice_for' AND post_meta.meta_key = 'smgt_class_id' AND
			(post_meta.meta_value = %d OR post_meta.meta_value = 'all')))",
			$role,
			$class_id
		);
		if ( ! empty( $notice_limit ) ) {
			$sql .= ' ' . $notice_limit;
		}
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
	...
	private function mjschool_payment( $user_role ) {
		...
		if ( $user_role === 'student' ) {
			...
			if ( ! empty( $this->child_list ) && is_array( $this->child_list ) ) {
				$child_ids    = array_map( 'intval', $this->child_list );
				$placeholders = implode( ', ', array_fill( 0, count( $child_ids ), '%d' ) );
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
				$result = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $table_name WHERE student_id IN ($placeholders)", ...$child_ids ) );
			} else {
				$result = array();
			}
		} else {
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
			$result = $wpdb->get_results( "SELECT p.* FROM $table_name AS p INNER JOIN $table_users AS u ON p.student_id = u.ID" );
		}
		return $result;
	}
	/**
	 ...
	 *
	 * @since 1.0.0
	 * @param int|array $class_id The class ID or array of class IDs.
	...
	public function mjschool_get_student_list( $class_id ) {
		...
		$student_access_data = isset( $student_access['student'] ) ? $student_access['student'] : array();
		$data                = array();
		...
		if ( $this->role === 'student' && isset( $data['own_data'] ) && $data['own_data'] === '1' ) {
			// phpcs:disable WordPress.DB.SlowDBQuery.slow_db_query_meta_query
			$students = get_users(
				array(
					'role'       => 'student',
