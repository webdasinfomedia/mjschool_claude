	...
	if ( wp_verify_nonce( $nonce, 'save_hostel_admin_nonce' ) ) {
		if ( isset($_REQUEST['action']) && sanitize_text_field(wp_unslash($_REQUEST['action'])) === 'edit' ) {
			if ( isset( $_GET['_wpnonce_action'] ) && wp_verify_nonce( $_GET['_wpnonce_action'], 'edit_action' ) ) {
				...
				wp_redirect( admin_url() . 'admin.php?page=mjschool_hostel&tab=hostel_list&message=2' );
				die();
			} else {
			...
			if ( $result ) {
				wp_redirect( admin_url() . 'admin.php?page=mjschool_hostel&tab=hostel_list&message=1' );
				die();
			}
		}
	}
...
if ( isset( $_REQUEST['action'] ) && sanitize_text_field(wp_unslash($_REQUEST['action'])) === 'delete' ) {
	if ( isset( $_GET['_wpnonce_action'] ) && wp_verify_nonce( $_GET['_wpnonce_action'], 'delete_action' ) ) {
		...
		if ( $result ) {
			wp_redirect( admin_url() . 'admin.php?page=mjschool_hostel&tab=hostel_list&message=3' );
			die();
		}
	} else {
		wp_die( esc_html__( 'Security check failed!', 'mjschool' ) );
				...
				<div class="mjschool-main-list-page"><!-- mjschool-main-list-page. -->
					...
					if ( $active_tab === 'hostel_list' ) {
						...
						if ( get_option( 'mjschool_enable_video_popup_show' ) == 'yes' ) {
							?>
							<a href="#" class="mjschool-view-video-popup youtube-icon" link="<?php echo 'https://www.youtube.com/embed/CZQzPhCPIr4?si=Hg16bHUL2gzi9xLA'; ?>" title="Hostel Module">
											...
											<tbody>
												...
												foreach ( $retrieve_class_data as $retrieved_data ) {
													...
													<tr>
														...
														<td>
															<a href="?page=mjschool_hostel&tab=hostel_details&hostel_id=<?php echo esc_attr( mjschool_encrypt_id( $retrieved_data->id ) ); ?>">
																<?php echo esc_attr( $retrieved_data->hostel_name ); ?> <i class="fa-solid fa-circle-info mjschool-fa-information-bg" data-toggle="tooltip" data-placement="top" title="<?php esc_attr_e( 'Hostel Name', 'mjschool' ); ?>"></i>
															</a>
														</td>
														<td>
															<?php
																		...
																		<li >
																			...
																			<ul class="dropdown-menu mjschool-header-dropdown-menu mjschool-action-dropdawn" aria-labelledby="dropdownMenuLink">
																				<li class="mjschool-float-left-width-100px">
																					<a href="?page=mjschool_hostel&tab=hostel_details&hostel_id=<?php echo esc_attr( mjschool_encrypt_id( $retrieved_data->id ) ); ?>" class="mjschool-float-left-width-100px"><i class="fas fa-eye"></i><?php esc_html_e( 'View', 'mjschool' ); ?></a>
																				</li>
																				<?php
																				if ( $user_access_edit === '1' ) {
																					?>
																					<li class="mjschool-float-left-width-100px mjschool-border-bottom-item">
																						<a href="?page=mjschool_hostel&tab=add_hostel&action=edit&hostel_id=<?php echo esc_attr( mjschool_encrypt_id( $retrieved_data->id ) ); ?>&_wpnonce_action=<?php echo esc_attr( mjschool_get_nonce( 'edit_action' ) ); ?>" class="mjschool-float-left-width-100px"><i class="fas fa-edit"></i><?php esc_html_e( 'Edit', 'mjschool' ); ?></a>
																					</li>
																					<?php
																				}
