...
class Mjschool_Homework {
	...
	public function mjschool_check_valid_extension( $filename ) {
		...
		if ( $filename !== '' ) {
			...
			if ( in_array( $ext, $valid_extension, true ) ) {
				$flag = 1;
			}
		}
		return $flag;
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_get_delete_records( $mjschool_table_name, $record_id ) {
		global $wpdb;
		...
		return $wpdb->query( $wpdb->prepare( "DELETE FROM $table_name WHERE homework_id= %d", intval( $record_id ) ) );
	}
	/**
	...
	public function mjschool_check_uploaded( $assign_id ) {
		...
		$query  = $wpdb->prepare( "SELECT file FROM {$table} WHERE stu_homework_id = %d", intval( $assign_id ) );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
	...
	public function mjschool_get_student_submitted_homework( $stu_homework_id ) {
		...
		$table = $wpdb->prefix . 'mjschool_student_homework';
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		$result = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $table WHERE stu_homework_id =%d", intval( $stu_homework_id ) ) );
		return $result;
	}
	/**
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_get_class_homework() {
		global $wpdb;
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_view_submission( $data ) {
		global $wpdb;
		$table_name  = $wpdb->prefix . 'mjschool_homework';
		$table_name2 = $wpdb->prefix . 'mjschool_student_homework';
		$query       = $wpdb->prepare( "SELECT * FROM {$table_name} AS a LEFT JOIN {$table_name2} AS b ON a.homework_id = b.homework_id WHERE a.homework_id = %d", intval( $data ) );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
	...
	public function mjschool_parent_view_detail() {
		...
		foreach ( $child as $student_id ) {
			...
			$class_id = intval( get_user_meta( intval( $student_id ), 'class_name', true ) );
			$query    = $wpdb->prepare(
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_parent_update_detail( $data, $student_id ) {
		global $wpdb;
		global $user_ID;
		$table_name  = $wpdb->prefix . 'mjschool_homework';
		$table_name2 = $wpdb->prefix . 'mjschool_student_homework';
		// Use prepared statement to securely query the database.
		$query = $wpdb->prepare( "SELECT * FROM {$table_name} AS a LEFT JOIN {$table_name2} AS b ON a.homework_id = b.homework_id WHERE a.homework_id = %d AND b.student_id = %d", intval( $data ), intval( $student_id ) );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		$result = $wpdb->get_results( $query );
		return $result;
	}
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_parent_update_detail_api( $data, $student_id ) {
		global $wpdb;
		global $user_ID;
		$table_name2 = $wpdb->prefix . 'mjschool_student_homework';
		$query       = $wpdb->prepare( "SELECT * FROM {$table_name2} WHERE student_id = %d AND homework_id = %d", intval( $student_id ), intval( $data ) );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
	...
	public function mjschool_add_homework( $data, $document_data ) {
		...
		$table_name                     = $wpdb->prefix . 'mjschool_homework';
		$table_name2                    = $wpdb->prefix . 'mjschool_student_homework';
		$homeworkdata['title']          = isset( $data['title'] ) ? sanitize_text_field( stripslashes( $data['title'] ) ) : '';
		$homeworkdata['class_name']     = isset( $data['class_name'] ) ? intval( $data['class_name'] ) : 0;
		$homeworkdata['section_id']     = isset( $data['class_section'] ) ? intval( $data['class_section'] ) : 0;
		$homeworkdata['subject']        = isset( $data['subject_id'] ) ? intval( $data['subject_id'] ) : 0;
		$homeworkdata['content']        = isset( $data['content'] ) ? stripslashes( sanitize_textarea_field( $data['content'] ) ) : '';
		$homeworkdata['marks']          = isset( $data['homework_marks'] ) ? floatval( $data['homework_marks'] ) : 0;
		$homeworkdata['created_date']   = gmdate( 'Y-m-d H:i:s' );
		$homeworkdata['submition_date'] = isset( $data['sdate'] ) ? gmdate( 'Y-m-d', strtotime( sanitize_text_field( $data['sdate'] ) ) ) : gmdate( 'Y-m-d' );
		...
		if ( ! empty( $_REQUEST['homework_id'] ) ) {
			...
			$homework_id['homework_id']        = intval( mjschool_decrypt_id( $homework_id_raw ) );
			$homeworkdata['homework_document'] = wp_json_encode( $document_data );
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
			$result = $wpdb->update( $table_name, $homeworkdata, $homework_id );
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
			$last_homework_id = $wpdb->insert_id;
			$homework         = $homeworkdata['title'];
			mjschool_append_audit_log( '' . esc_html__( 'Homework Updated', 'mjschool' ) . '( ' . esc_html( $homework ) . ' )' . '', get_current_user_id(), get_current_user_id(), 'edit', $page_name );
			if ( $result ) {
				$exlude_id = mjschool_approve_student_list();

				if ( ! empty( $data['class_section'] ) ) {
					$class_id    = intval( $data['class_name'] );
					...
					$studentdata = get_users(
						array(
							...
							'meta_value' => intval( $data['class_section'] ),
							'meta_query' => array( array( 'key' => 'class_name', 'value' => intval( $data['class_name'] ), 'compare' => '=' ) ),
							'role'       => 'student',
							'exclude'    => $exlude_id,
						)
					);
			...
			$last_homework_id = $wpdb->insert_id;
			$homework         = $homeworkdata['title'];
			mjschool_append_audit_log( '' . esc_html__( 'Homework Added', 'mjschool' ) . '( ' . esc_html( $homework ) . ' )' . '', get_current_user_id(), get_current_user_id(), 'insert', $page_name );
			if ( $result ) {
				$exlude_id = mjschool_approve_student_list();
				...
				if ( ! empty( $studentdata ) ) {
					...
					$homeworstud['created_date'] = gmdate( 'Y-m-d H:i:s' );
					$device_token                = array();
					foreach ( $studentdata as $student ) {
						$homeworstud['student_id'] = $student->ID;
						// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
					...
					$title             = esc_html__( 'New Notification For Homework', 'mjschool' );
					$text              = esc_html__( 'New homework has been assign to you', 'mjschool' );
					...
					if ( $insert ) {
						$enable_mail    = isset( $data['mjschool_enable_homework_mail'] ) && $data['mjschool_enable_homework_mail'] === '1';
						$enable_student = isset( $data['mjschool_enable_homework_mjschool_student'] ) && $data['mjschool_enable_homework_mjschool_student'] === '1';
						$enable_parent  = isset( $data['mjschool_enable_homework_mjschool_parent'] ) && $data['mjschool_enable_homework_mjschool_parent'] === '1';
						if ( $enable_mail || $enable_student || $enable_parent ) {
							foreach ( $studentdata as $userdata ) {
								...
								if ( $enable_mail ) {
									if ( ! empty( $parent ) ) {
										foreach ( $parent as $p ) {
											...
											$searchArr['{{title}}']                = sanitize_text_field( $data['title'] );
											$searchArr['{{submition_date}}']       = mjschool_get_date_in_input_box( sanitize_text_field( $data['sdate'] ) );
											$searchArr['{{homework_date}}']        = mjschool_get_date_in_input_box( gmdate( 'Y-m-d H:i:s' ) );
											...
											if ( ! empty( $document_data[0] ) ) {
												$attechment = WP_CONTENT_DIR . '/uploads/school_assets/' . sanitize_file_name( $document_data[0]['value'] );
											} else {
												$attechment = '';
											}
									...
									$string['{{title}}']          = sanitize_text_field( $data['title'] );
									$string['{{submition_date}}'] = mjschool_get_date_in_input_box( sanitize_text_field( $data['sdate'] ) );
									$string['{{homework_date}}']  = mjschool_get_date_in_input_box( gmdate( 'Y-m-d H:i:s' ) );
								...
								if ( $enable_student ) {
									...
									$SMSArr['{{title}}']        = sanitize_text_field( $data['title'] );
									$SMSArr['{{date}}']         = mjschool_get_date_in_input_box( sanitize_text_field( $data['sdate'] ) );
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_get_all_homework_list() {
		global $wpdb;
		$table_name = $wpdb->prefix . 'mjschool_homework';
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		return $wpdb->get_results( "SELECT * from $table_name ORDER BY submition_date DESC" );
	}
	/**
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_get_all_own_homeworklist() {
		global $wpdb;
		$get_current_user_id = get_current_user_id();
		$table_name          = $wpdb->prefix . 'mjschool_homework';
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		return $wpdb->get_results( $wpdb->prepare( "SELECT * from $table_name where createdby =%d ORDER BY submition_date DESC", $get_current_user_id ) );
	}
	/**
	...
	public function mjschool_get_all_own_upcoming_homeworklist() {
		...
		$table_name          = $wpdb->prefix . 'mjschool_homework';
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		return $wpdb->get_results( $wpdb->prepare( "SELECT * from $table_name where createdby =%d AND submition_date >= %s ORDER BY submition_date DESC", $get_current_user_id, $current_date ) );
	}
	/**
	 ...
	 *
	 * Uses prepared statements with placeholders for security.
	 *
	 * @global wpdb $wpdb WordPress database access abstraction object.
	 * @return array Array of homework objects assigned to the teacher's classes.
	 * @since  1.0.0
	 */
	public function mjschool_get_teacher_homeworklist() {
		global $wpdb;
		$table_name = $wpdb->prefix . 'mjschool_homework';
		$class_name = get_user_meta( get_current_user_id(), 'class_name', true );
		
		// Ensure we have an array of class IDs
		if ( empty( $class_name ) || ! is_array( $class_name ) ) {
			return array();
		}
		
		// Sanitize all class IDs to integers
		$class_ids = array_map( 'intval', $class_name );
		
		// Create placeholders for prepared statement
		$placeholders = implode( ', ', array_fill( 0, count( $class_ids ), '%d' ) );
		
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		return $wpdb->get_results( $wpdb->prepare( "SELECT * from $table_name where class_name IN( $placeholders )", ...$class_ids ) );
	}
	/**
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_get_edit_record( $homework_id ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'mjschool_homework';
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		return $wpdb->get_row( $wpdb->prepare( "SELECT * from $table_name where homework_id=%d", intval( $homework_id ) ) );
	}
	/**
	 ...
	 * @since  1.0.0
	 */
	public function mjschool_get_delete_record( $homework_id ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'mjschool_homework';
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		$home      = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $table_name where homework_id= %d ", intval( $homework_id ) ) );
		$homework  = isset( $home->title ) ? $home->title : esc_html__( 'Unknown Homework', 'mjschool' );
		$page_name = isset( $_REQUEST['page'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['page'] ) ) : '';
		mjschool_append_audit_log( '' . esc_html__( 'Homework Deleted', 'mjschool' ) . '( ' . esc_html( $homework ) . ' )' . '', get_current_user_id(), get_current_user_id(), 'delete', $page_name );
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
		return $wpdb->query( $wpdb->prepare( "Delete from $table_name where homework_id= %d ", intval( $homework_id ) ) );
	}
	/**
	...
	public function mjschool_get_all_own_homework_list_for_teacher() {
		...
		if ( ! empty( $class ) && is_array( $class ) ) {
			foreach ( $class as $class_id ) {
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
				$rows[] = $wpdb->get_results( $wpdb->prepare( "SELECT * from $table_name where class_name = %d ORDER BY submition_date DESC", intval( $class_id ) ) );
			}
		}
		if ( ! empty( $rows ) ) {
	...
	public function mjschool_get_all_own_upcoming_homework_list_for_teacher() {
		...
		if ( ! empty( $class ) && is_array( $class ) ) {
			foreach ( $class as $class_id ) {
				// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
				$rows[] = $wpdb->get_results( $wpdb->prepare( "SELECT * from $table_name where class_name = %d AND submition_date >= %s ORDER BY submition_date DESC", intval( $class_id ), $current_date ) );
			}
		}
		if ( ! empty( $rows ) ) {
			$mergedArray         = array_merge( ...$rows );
	 ...
	 */
	public function mjschool_update_student_homework(
		$stud_homework_id,
		$file_name,
		$obtain_marks,
		$teacher_comment,
		...
		// Update data - sanitize all inputs
		$data = array(
			'review_file'     => sanitize_file_name( $file_name ),
			'obtain_marks'    => floatval( $obtain_marks ),
			'teacher_comment' => sanitize_textarea_field( $teacher_comment ),
			'evaluate_date'   => sanitize_text_field( $evaluate_date ),
		...
		// WHERE clause
		$where        = array( 'stu_homework_id' => intval( $stud_homework_id ) );
