...
$invoice_type = isset( $_GET['invoice_type'] ) ? sanitize_text_field( wp_unslash( $_GET['invoice_type'] ) ) : '';
$idtest = isset( $_GET['idtest'] ) ? sanitize_text_field( wp_unslash( $_GET['idtest'] ) ) : '';
if ( $invoice_type === 'invoice' ) {
    $invoice_data = $mjschool_obj_invoice->mjschool_get_payment_by_id( intval( mjschool_decrypt_id( $idtest ) ) );
}
								...
								<table class="table table-bordered mjschool-model-invoice-table">
									...
									<tbody>
										...
										if ( ! empty( $income_data ) || ! empty( $expense_data ) ) {
											...
											$all_entries = $mjschool_obj_invoice->mjschool_get_one_student_income_data( mjschool_decrypt_id( $idtest ) );
								...
								<table class="table mjschool-model-invoice-table">
									...
									<tbody>
										...
										if ( ! empty( $income_data ) || ! empty( $expense_data ) ) {
											...
											$patient_all_income = $mjschool_obj_invoice->mjschool_get_one_student_income_data( mjschool_decrypt_id( $idtest ) );
