...
if ( isset( $_POST['save_parent'] ) ) {
	...
	if ( wp_verify_nonce( $nonce, 'save_parent_admin_nonce' ) ) {
		...
		if ( isset( $_POST['mjschool_user_avatar'] ) && $_POST['mjschool_user_avatar'] !== '' ) {
			$photo = esc_url_raw( wp_unslash( $_POST['mjschool_user_avatar'] ) );
		} else {
			$photo = '';
		}
		// DOCUMENT UPLOAD
		...
		$usermetadata   = array(
			...
			'city'                 => isset( $_POST['city_name'] ) ? sanitize_text_field( wp_unslash( $_POST['city_name'] ) ) : '',
			'state'                => isset( $_POST['state_name'] ) ? sanitize_text_field( wp_unslash( $_POST['state_name'] ) ) : '',
		...
		if ( $current_action === 'edit' ) {
			if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'edit_action' ) ) {
				...
				if ( $result ) {
					wp_safe_redirect( admin_url( 'admin.php?page=mjschool_parent&tab=parentlist&message=1' ) );
					exit;
				}
			} else {
			...
			if ( $result ) {
				wp_safe_redirect( admin_url( 'admin.php?page=mjschool_parent&tab=parentlist&message=2' ) );
				exit;
			}
		} else {
			wp_safe_redirect( admin_url( 'admin.php?page=mjschool_parent&tab=parentlist&message=3' ) );
			exit;
		}
	}
}
	...
	if ( isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'delete_action' ) ) {
		...
		if ( ! empty( $childs ) ) {
			foreach ( $childs as $childvalue ) {
				...
				if ( ! empty( $parents ) ) {
					$key = array_search( $parent_id, $parents, true );
					if ( $key !== false ) {
						unset( $parents[ $key ] );
		...
		foreach ( $ids_array as $id ) {
			...
			if ( ! empty( $childs ) ) {
				foreach ( $childs as $childvalue ) {
					$parents = get_user_meta( $childvalue, 'parent_id', true );
					if ( ! empty( $parents ) ) {
						$key = array_search( $id, $parents, true );
						if ( $key !== false ) {
							unset( $parents[ $key ] );
							update_user_meta( $childvalue, 'parent_id', $parents );
						}
...
if ( isset( $_POST['parent_export_csv_selected'] ) ) {
	if ( isset( $_POST['id'] ) && is_array( $_POST['id'] ) ) {
		...
		if ( ! empty( $staff_list ) ) {
			...
			fclose( $fh );
			ob_clean();
			$file = MJSCHOOL_PLUGIN_DIR . '/sample-csv/export/mjschool-export-parent.csv';
			$mime = 'text/plain';
			...
			echo '<div style="background: none repeat scroll 0 0 red;border: 1px solid;color: white;float: left;font-size: 17px;margin-top: 10px;padding: 10px;width: 98%;">' . esc_html__( 'Records not found.', 'mjschool' ) . '</div>';
		}
	}
}
	...
	if ( wp_verify_nonce( $nonce, 'upload_csv_nonce' ) ) {
		if ( isset( $_FILES['csv_file'] ) ) {
			...
			if ( ! in_array( $file_ext, $extensions, true ) ) {
				...
				$errors[] = esc_html__( 'This file not allowed, please choose a CSV file.', 'mjschool' );
				wp_safe_redirect( admin_url( 'admin.php?page=mjschool_parent&tab=uploadparent&message=6' ) );
				exit;
			}
			if ( $file_size > 2097152 ) {
				$errors[] = 'File size limit 2 MB';
				wp_safe_redirect( admin_url( 'admin.php?page=mjschool_parent&tab=uploadparent&message=7' ) );
				exit;
			}
			if ( empty( $errors ) ) {
				...
				foreach ( $rows as $row ) {
					...
					if ( is_wp_error( $user_id ) ) {
						...
						mjschool_append_csv_log( $log_message, get_current_user_id(), $module, $status );
						echo '<script type="text/javascript">alert("Problems with user: ' . esc_js( $username ) . ', we are going to skip");</script>';
						continue;
					}
					if ( $mjschool_role_name !== 'administrator' ) {
						wp_update_user(
					...
					if ( isset( $csv['user_profile'] ) ) {
						$upload_dir = wp_upload_dir();
						$photo      = $upload_dir['baseurl'] . '/' . sanitize_text_field( $csv['user_profile'] );
						update_user_meta( $user_id, 'mjschool_user_avatar', esc_url_raw( $photo ) );
					}
					$alt_number = '';
					...
					if ( isset( $csv['child'] ) ) {
						...
						foreach ( $child_username as $child_email ) {
							$mjschool_user_child = get_user_by( 'email', sanitize_email( trim( $child_email ) ) );
							if ( $mjschool_user_child ) {
								$student_id   = $mjschool_user_child->data->ID;
								...
								if ( $student_data ) {
									if ( ! in_array( $user_id, $student_data, true ) ) {
										array_push( $student_data, $user_id );
										update_user_meta( $student_id, 'parent_id', $student_data );
									}
								} else {
									$parent_id_arr = array( $user_id );
									add_user_meta( $student_id, 'parent_id', $parent_id_arr );
								}
								if ( $parent_data ) {
									if ( ! in_array( $student_id, $parent_data, true ) ) {
										array_push( $parent_data, $student_id );
										update_user_meta( $user_id, 'child', $parent_data );
									}
								} elseif ( ! empty( $student_id ) ) {
									$child_id_arr = array( $student_id );
									add_user_meta( $user_id, 'child', $child_id_arr );
								}
							}
						}
					}
					...
					if ( $user_created ) {
						...
						if ( $import_mail === '1' ) {
							if ( $user_id ) {
								...
								mjschool_send_mail( $email_to, $MsgSubject, $message );
							}
						}
						$module      = 'parent';
						$status      = 'Success';
			...
			if ( isset( $success ) ) {
				wp_safe_redirect( admin_url( 'admin.php?page=mjschool_parent&tab=parentlist&message=5' ) );
				exit;
			}
		}
	} else {
