...
<div class="mjschool-page-inner"><!--------- Page Inner. ------->
	<div class="mjschool-main-list-margin-5px">
			...
			if ( wp_verify_nonce( $nonce, 'save_leave_nonce' ) ) {
				if ( isset( $_REQUEST['action'] ) && sanitize_text_field(wp_unslash($_REQUEST['action'])) === 'edit' ) {
					...
					if ( isset( $_GET['_wpnonce_action'] ) && wp_verify_nonce( $_GET['_wpnonce_action'], 'edit_action' ) ) {
						...
						wp_redirect( admin_url() . 'admin.php?page=mjschool_leave&tab=leave_list&message=2' );
						die();
					} else {
					...
					if ( $result ) {
						wp_redirect( admin_url() . 'admin.php?page=mjschool_leave&tab=leave_list&message=1' );
						die();
					}
				}
			}
		...
		if ( isset( $_POST['approve_comment'] ) ) {
			...
			if ( $result ) {
				wp_redirect( admin_url() . 'admin.php?page=mjschool_leave&tab=leave_list&message=4' );
				die();
			} else {
				wp_redirect( admin_url() . 'admin.php?page=mjschool_leave&tab=leave_list&message=6' );
				die();
			}
		}
		if ( isset( $_POST['reject_leave'] ) ) {
			...
			if ( $result ) {
				wp_redirect( admin_url() . 'admin.php?page=mjschool_leave&tab=leave_list&message=5' );
				die();
			} else {
				wp_redirect( admin_url() . 'admin.php?page=mjschool_leave&tab=leave_list&message=6' );
				die();
		...
		if ( isset( $_REQUEST['action'] ) && sanitize_text_field(wp_unslash($_REQUEST['action'])) === 'delete' ) {
			...
			if ( $result ) {
				wp_redirect( admin_url() . 'admin.php?page=mjschool_leave&tab=leave_list&message=3' );
				die();
			}
		}
									...
									<div class="col-md-3 input">
										<select class="form-control Student_leave_drop" id="Student_leave" name="Student_id">
											...
											foreach ( $studentdata as $student ) {
												...
												?>
												<option value="<?php print esc_attr( $student->ID ); ?>" <?php selected( $student->ID, $emp_id ); ?>><?php echo esc_attr( mjschool_student_display_name_with_roll( $student->ID ) ); ?></option>
												<?php
											}
											?>
										</select>
								...
								<form id="mjschool-common-form" name="mjschool-common-form" method="post">
									<table id="leave_list" class="display mjschool-admin-transport-datatable" cellspacing="0" width="100%">
										...
										<tbody>
											...
											foreach ( $leave_data as $retrieved_data ) {
												...
												?>
												<tr>
													<td class="mjschool-checkbox-width-10px"><input type="checkbox" class="mjschool-sub-chk selected_leave select-checkbox" name="id[]" value="<?php echo esc_attr( $retrieved_data->id ); ?>"></td>
																	...
																	<ul class="dropdown-menu mjschool-header-dropdown-menu mjschool-action-dropdawn" aria-labelledby="dropdownMenuLink">
																		...
																		if ( ( $retrieved_data->status != 'Approved' ) ) {
																			...
																			<li class="mjschool-float-left-width-100px mjschool-border-bottom-menu">
																				<a href="#" leave_id="<?php echo esc_attr( $retrieved_data->id ); ?>" class="mjschool-float-left-width-100px leave-approve mjschool_height_17px">
																		...
																		if ( $mjschool_role === 'administrator' ) {
																			?>
																			<li class="mjschool-float-left-width-100px mjschool-border-bottom-menu">
																				<a href="?page=mjschool_leave&tab=add_leave&action=edit&leave_id=<?php echo esc_attr( $leave_id ); ?>&_wpnonce_action=<?php echo esc_attr( mjschool_get_nonce( 'edit_action' ) ); ?>" class="mjschool-float-left-width-100px"><i class="fas fa-edit"> </i><?php esc_html_e( 'Edit', 'mjschool' ); ?></a>
																			</li>
																			<li class="mjschool-float-left-width-100px">
																				<a href="?page=mjschool_leave&tab=leave_list&action=delete&leave_id=<?php echo esc_attr( $leave_id ); ?>&_wpnonce_action=<?php echo esc_attr( mjschool_get_nonce( 'delete_action' ) ); ?>" class="mjschool-float-left-width-100px mjschool_orange_color" onclick="return confirm( '<?php esc_html_e( 'Are you sure you want to delete this record?', 'mjschool' ); ?>' );">
																			...
																			?>
																			<li class="mjschool-float-left-width-100px mjschool-border-bottom-menu">
																				<a href="?dashboard=mjschool_user&page=leave&tab=add_leave&action=edit&leave_id=<?php echo esc_attr( $leave_id ); ?>&_wpnonce_action=<?php echo esc_attr( mjschool_get_nonce( 'edit_action' ) ); ?>" leave_id="<?php echo esc_attr( $retrieved_data->id ); ?>" class="mjschool-float-left-width-100px leave-reject"><i class="fas fa-edit"> </i><?php esc_html_e( 'Edit', 'mjschool' ); ?></a>
																			</li>
																			<li class="mjschool-float-left-width-100px">
									...
									<div class="mjschool-print-button pull-left">
										<button class="mjschool-btn-sms-color mjschool-button-reload">
											<input type="checkbox" id="select_all" class="mjschool-sub-chk select_all mjchool_margin_top_0px" name="id[]" value="<?php echo esc_attr( $retrieved_data->id ); ?>" >
