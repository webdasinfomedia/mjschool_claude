...
class Smgt_paypal_class {
	...
	public function submit_paypal_post() {
		...
		if ( get_option( 'mjschool_enable_sandbox' ) == 'yes' ) {
			$paypal_account_type = 1;// 1 sendbox, 0 live;
		}
		// $paypal_account_type = test_paypal; //1 sendbox, 0 live;
		$paypal_url = $paypal_account_type ? MJSCHOOL_SSL_SAND_URL : MJSCHOOL_SSL_P_URL;
		echo "<html>\n";
		echo "<head><title>Processing Payment...</title></head>\n";
		echo "<body onLoad=\"document.forms['paypal_form'].on( 'submit', function(e) { e.preventDefault(); });\">\n";
	...
	public function validate_ipn() {
		...
		$paypal_url = ( $_POST['test_ipn'] == 1 ) ? MJSCHOOL_SSL_SAND_URL : MJSCHOOL_SSL_P_URL;
	...
	private function write_to_log() {
		...
		// Write to log
		$fp = fopen( MJSCHOOL_LOG_FILE, 'a' );
		fwrite( $fp, $this->ipn_status . "\n\n" );
		fclose( $fp ); // close file
		chmod( MJSCHOOL_LOG_FILE, 0600 );
	}
	...
	public function dump_fields() {
		...
		ksort( $this->fields );
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Safe internal data; no user input
		foreach ( $this->fields as $key => $value ) {
			echo "<tr><td>$key</td><td>" . urldecode( $value ) . '&nbsp;</td></tr>'; //phpcs:ignore
		}
		echo '</table><br>';
	}
