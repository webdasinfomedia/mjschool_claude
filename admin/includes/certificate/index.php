...
if ( isset( $_POST['create_exprience_latter']) || isset($_POST['save_and_print'] ) ) {
    if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'mjschool_assign_certificate_nonce' ) ) {
    ...
    if ( isset( $_POST['edit'] ) ) {
        ...
        $latter_id = intval( wp_unslash($_POST['id'] ) );
        if ( $result ) {
...
if ( isset( $_REQUEST['delete_selected'] ) ) {
    if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'mjschool_delete_certificate_nonce' ) ) {
        wp_die( esc_html__( 'Security check failed.', 'mjschool' ) );
    }
...
if ( isset( $_REQUEST['save_transfer'] ) ) {
    if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'mjschool_save_transfer_certificate_nonce' ) ) {
        wp_die( esc_html__( 'Security check failed.', 'mjschool' ) );
    }
...
if ( isset( $_POST['save_dynamic_certificate']) && isset($_POST['id'] ) ) {
    if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'mjschool_save_dynamic_certificate_nonce' ) ) {
        wp_die( esc_html__( 'Security check failed.', 'mjschool' ) );
    }
...
if ( isset( $_GET['delete_certificate'] ) ) {
    if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'mjschool_delete_certificate_action' ) ) {
        wp_die( esc_html__( 'Security check failed.', 'mjschool' ) );
    }
    ...
    wp_safe_redirect( admin_url( 'admin.php?page=mjschool_certificate&tab=certificatelist&_wpnonce=' . rawurlencode( $delete_nonce ) . '&message=1' ) );
    exit;
}
?>
    ...
    <div class="mjschool-panel-body">
        ...
        if ( $active_tab === 'certificatelist' ) {
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group input">
                                                    <div class="col-md-12 form-control">
                                                        <input class="form-control validate[required]" type="text" name="mjschool_transfer_certificate_title" id="mjschool_transfer_certificate_title" placeholder="Enter Title" value="<?php echo esc_attr( get_option( 'mjschool_transfer_certificate_title' ) ); ?>">
                                            ...
                                            if ( $latter_access_edit === '1' ) {
                                                ...
                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                    <input value="<?php esc_attr_e( 'Save', 'mjschool' ); ?>" name="save_transfer" class="btn btn-success mjschool-save-btn" type="submit">
                                                </div>
                                                <?php
                                            }
                                            ?>
            ...
            if ( isset( $_POST['clone_cert'] ) ) {
                if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'mjschool_clone_certificate_nonce' ) ) {
                    wp_die( esc_html__( 'Security check failed.', 'mjschool' ) );
                }
            ...
            // Loop over dynamically created certificates like sample_certificate_1, sample_certificate_2, etc.
            global $wpdb;
            ++$i;
            ...
            foreach ( $results as $cert ) {
                ...
                <div class="d-flex justify-content-start mb-2 mt-1">
                    <a class="mjschool-dropdown-icon-letter" data-toggle="tooltip" href="<?php echo esc_url( admin_url( 'admin.php?page=mjschool_certificate&tab=assign_certificate&letter_type=' . rawurlencode( sanitize_text_field( $title ) ) . '&action=new' ) ); ?>" title="<?php esc_attr_e( 'Assign Certificate', 'mjschool' ); ?>">
                        <img height="41px" src="<?php echo esc_url( MJSCHOOL_PLUGIN_URL . "/assets/images/dashboard-icon/mjschool-addmore-icon.png"); ?>" alt="<?php esc_attr_e( 'Assign', 'mjschool' ); ?>" class="add_more_icon_detailpage">
                    </a>
                    <a class="mjschool-dropdown-icon-letter" data-toggle="tooltip" href="<?php echo esc_url( admin_url( 'admin.php?page=mjschool_certificate&tab=certificatelist&delete_certificate=' . rawurlencode( sanitize_text_field( $cert->id ) ) . '&_wpnonce=' . rawurlencode( $delete_cert_nonce ) ) ); ?>" onclick="return confirm( '<?php esc_js( esc_attr__( 'Are you sure you want to delete this certificate?', 'mjschool' ) ); ?>' );" title="<?php esc_attr_e( 'Delete Certificate', 'mjschool' ); ?>">
                        <img src="<?php echo esc_url( MJSCHOOL_PLUGIN_URL . "/assets/images/listpage-icon/mjschool-delete.png"); ?>" alt="<?php esc_attr_e( 'Delete', 'mjschool' ); ?>" class="add_more_icon_detailpage">
                    </a>
                </div>
                <div class="mjschool-main-email-template">
                                    ...
                                    <form class="mjschool-form-horizontal" method="post">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group input">
                                                    <div class="col-md-12 form-control">
                                                        <input class="form-control" type="text" name="<?php echo esc_attr($prefix . $index); ?>_title" placeholder="<?php esc_attr_e( 'Enter Title', 'mjschool' ); ?>" value="<?php echo esc_attr($title); ?>">
                                            ...
                                            if ( $latter_access_edit === '1' ) {
                                                ?>
                                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                                    <input value="<?php esc_attr_e( 'Save', 'mjschool' ); ?>" name="save_dynamic_certificate" class="btn btn-success mjschool-save-btn" type="submit">
                                                </div>
                                                <?php
                                            }
                                            ?>
        ...
        if ( $active_tab === 'view_assign_certificate' ) {
            require_once MJSCHOOL_ADMIN_DIR . '/certificate/view-certificate.php';
        }
        ?>
    </div>
</div>