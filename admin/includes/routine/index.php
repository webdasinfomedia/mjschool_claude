...
if ( isset( $_POST['save_route'] ) ) {
	...
	if (wp_verify_nonce($nonce, 'save_root_admin_nonce' ) ) {
		...
		$start_time         = mjschool_time_convert(sanitize_text_field($_POST['start_time'] ) );
		$end_time           = mjschool_time_convert(sanitize_text_field($_POST['end_time'] ) );
		$start_time_1       = sanitize_text_field($_POST['start_time']);
		$end_time_1         = sanitize_text_field($_POST['end_time']);
		$start_time_convert = date( 'h:i', strtotime(sanitize_text_field($_POST['start_time'] ) ) );
		$end_time_convert   = date( 'h:i', strtotime(sanitize_text_field($_POST['end_time'] ) ) );
		...
		{
			if ( $action === 'edit' ) {
				$route_data['subject_id']   = sanitize_text_field($_POST['subject_id']);
				$route_data['class_id']     = sanitize_text_field($_POST['class_id']);
				$route_data['section_name'] = sanitize_text_field($_POST['class_section']);
				$route_data['teacher_id']   = $teacher_id;
				$route_data['start_time']   = $start_time_new;
				$route_data['end_time']     = $end_time_new;
				$route_data['weekday']      = sanitize_text_field($_POST['weekday']);
				$route_data['multiple_teacher'] = 'yes';
				$route_data['room_id'] = sanitize_text_field($_POST['room_id']);
			} else {
				foreach ($teacher_id as $teacher) {
					foreach ($_POST['weekday'] as $week_days) {
						$route_data[] = array(
							...
							'section_name' => sanitize_text_field($_POST['class_section']),
							'teacher_id' => $teacher,
			...
			if ( $action === 'edit' ) //------- Edit class routine. --------//
			{
				if ( isset( $_GET['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'edit_action' ) ) {
					$route_id = array( 'route_id' => mjschool_decrypt_id(sanitize_text_field(wp_unslash($_REQUEST['route_id'])) ) );
					$mjschool_obj_route->mjschool_update_route($route_data, $route_id);
					
					wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=2' );
					die();
				} else {
					...
					if ($route_id_array) {
						foreach ($route_id_array as $route_id) {
							if ( $_POST['create_virtual_classroom'] === 1) {
								...
								$start_time = mjschool_start_time_convert(sanitize_text_field(wp_unslash($_POST['start_time'])));
								$end_time   = mjschool_end_time_convert(sanitize_text_field(wp_unslash($_POST['end_time'])));
						...
						wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=1' );
						die();
					}
				} elseif ($retuen_val === 'duplicate' ) {
					wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=4' );
					die();
				} elseif ($retuen_val === 'teacher_duplicate' ) {
					wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=5' );
					die();
				}
			}
		} else {
			wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=6' );
			die();
		}
	}
}
...
if ( isset( $_POST['save_import_csv'] ) ) {
	if ( isset( $_FILES['csv_file'] ) ) {
		...
		if (in_array($file_ext, $extensions) === false) {
			...
			$errors[] = $err;
			wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=import_class_route&_wpnonce='.esc_attr( $nonce ).'&message=7' );
			die();
		}
		//------------ Check File Size. ------------//
		if ($file_size > 2097152 ) {
			$errors[] = 'File size limit 2 MB';
			wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=import_class_route&_wpnonce='.esc_attr( $nonce ).'&message=8' );
			die();
		}
		if (empty($errors) === true) {
			...
			foreach ($rows as $row) {
				...
				$subject_code        = $csv['subject id'];
				$subject_name        = isset($csv['subject name']) ? $csv['subject name'] : '';
				...
				if ((int) $_POST['class_id'] === (int) $subject_data->class_id) {
			...
			wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=10' );
			die();
		}
	}
}
...
if ( isset( $_POST['create_meeting'] ) ) {
	...
	if (wp_verify_nonce($nonce, 'create_meeting_admin_nonce' ) ) {
		...
		if ($result) {
			wp_redirect(admin_url() . 'admin.php?page=mjschool_virtual_classroom&tab=meeting_list&_wpnonce='.esc_attr( $nonce ).'&message=1' );
			die();
		}
	}
}
...
if ( $action === 'delete' ) {
	if ( isset( $_GET['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'delete_action' ) ) {
		...
		$result = mjschool_delete_route($tablenm, mjschool_decrypt_id(sanitize_text_field(wp_unslash($_REQUEST['route_id'])) ) );
		if ($result) {
			$nonce = wp_create_nonce( 'mjschool_class_routine_tab' );
			wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=3' );
			die();
		}
	} else {
		wp_die( esc_html__( 'Security check failed!', 'mjschool' ) );
...
if ( $action === "routine_export_csv") {
	...
	$section_name  = sanitize_text_field( wp_unslash( $_REQUEST['class_section'] ) );
	if ( $class_id !== "" && ( $section_name === "remove" || $section_name === "" ) ) //------- Only Class Select -------//
	{
		$class_route_list = mjschool_get_time_table_using_class_and_section($class_id, 0);
	} else {
		$class_route_list = mjschool_get_time_table_using_class_and_section($class_id, $section_name);
	}
	if ( ! empty( $class_route_list ) ) {
		...
		wp_redirect(admin_url() . 'admin.php?page=mjschool_route&tab=route_list&_wpnonce='.esc_attr( $nonce ).'&message=9' );
		die;
	}
}
?>
						...
						if ( $action === 'edit' && $active_tab === 'addroute' ) { ?>
							...
							<li class="<?php if ($active_tab === 'addroute' ) { ?>active<?php } ?>">
								<a href="?page=mjschool_library&tab=addbook" class="mjschool-padding-left-0 tab <?php echo esc_attr( $active_tab  ) === 'addroute' ? 'nav-tab-active' : ''; ?>">
									<?php echo esc_attr__( 'Add Class Time Table', 'mjschool' ); ?>
								</a>
							</li>
							<?php
						}
													...
													<h4 class="accordion-header" id="flush-heading<?php echo esc_attr($i); ?>">
														<button class="accordion-button class_route_list collapsed " type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse_collapse_<?php echo esc_attr($i); ?>" aria-controls="flush-heading<?php echo esc_attr($i); ?>">
															<div class="col-md-10 col-7 mjschool-route-rtl-list">
																<span class="Title_font_weight"><?php echo esc_attr__( 'Class', 'mjschool' ) . ":"; ?>&nbsp;</span><?php echo esc_attr($class->class_name); ?>
															</div>
																				...
																				foreach ( $period as $period_data ) {
																					...
																					if ( get_option( 'mjschool_enable_virtual_classroom' ) === 'yes' ) {
																						...
																						if ( empty( $meeting_data ) ) {
																							$create_meeting = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none mjschool-show-virtual-popup" href="#" id="' . esc_attr( $period_data->route_id ) . '">' . esc_attr__( 'Create Virtual Class', 'mjschool' ) . '</a></li>';
																						} else {
																							$create_meeting = '';
																						}
																						if ( ! empty( $meeting_data ) ) {
																							$update_meeting      = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="admin.php?page=mjschool_virtual_classroom&tab=edit_meeting&action=edit&meeting_id=' . esc_attr( $meeting_data->meeting_id ) . '">' . esc_attr__( 'Edit Virtual Class', 'mjschool' ) . '</a></li>';
																							$delete_meeting      = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="admin.php?page=mjschool_virtual_classroom&tab=meeting_list&action=delete&meeting_id=' . esc_attr( $meeting_data->meeting_id ) . '" onclick="return confirm(\'' . esc_attr__( 'Are you sure you want to delete this record?', 'mjschool' ) . '\' );">' . esc_attr__( 'Delete Virtual Class', 'mjschool' ) . '</a></li>';
																							$meeting_statrt_link = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="' . $meeting_data->meeting_start_link . '" target="_blank">' . esc_attr__( 'Virtual Class Start', 'mjschool' ) . '</a></li>';
																						} else {
																							$update_meeting      = '';
																							$delete_meeting      = '';
																							$meeting_statrt_link = '';
																					...
																					echo '<ul role="menu" class="pt-2 dropdown-menu">
																							<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="?page=mjschool_route&tab=addroute&action=edit&route_id=' . esc_attr( $route_id ) . '&_wpnonce=' . esc_attr( mjschool_get_nonce( 'edit_action' ) ) . '">' . esc_attr__( 'Edit Time Table', 'mjschool' ) . '</a></li>
																							
																							<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="?page=mjschool_route&tab=route_list&action=delete&route_id=' . esc_attr( $route_id ) . '&_wpnonce=' . esc_attr( mjschool_get_nonce( 'delete_action' ) ) . '" onclick="return confirm(\'' . esc_attr__( 'Are you sure you want to delete this record?', 'mjschool' ) . '\' );">' . esc_attr__( 'Delete Time Table', 'mjschool' ) . '</a></li>' . wp_kses_post( $create_meeting ) . '' . wp_kses_post( $update_meeting ) . '' . wp_kses_post( $delete_meeting ) . '' . wp_kses_post( $meeting_statrt_link ) . '
																						</ul>';
																					echo '</div>';
																				}
																			}
													...
													<div class="mt-1 accordion-item mjschool-class-border-div">
														<h4 class="accordion-header" id="flush-heading<?php echo esc_attr( $i ); ?>">
															<button class="accordion-button class_route_list collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse_<?php echo esc_attr( $i ); ?>" aria-controls="flush-collapse_<?php echo esc_attr( $i ); ?>">
																<div class="col-md-10 col-7 mjschool-route-rtl-list">
																	<span class="Title_font_weight"><?php echo esc_attr__( 'Class', 'mjschool' ) . ':'; ?>&nbsp;</span><?php echo esc_html( mjschool_get_class_section_name_wise( $section->class_id, $sectionid ) ); ?> &nbsp;&nbsp;&nbsp;&nbsp;
																</div>
																<div class="col-md-2 col-5 row justify-content-end mjschool-view-result">
																	<div class="col-md-5 mjschool-width-50px">
																	...
																	<div class="col-md-4 mjschool-width-50px mjschool-rtl-margin-left-20px mjschool-exam-result-pdf-margin mjschool_margin_right_22px" >
																		<a href="?page=mjschool_route&tab=route_list&action=routine_export_csv&class_id=<?php echo esc_attr($class->class_id); ?>&class_section=<?php echo esc_attr($sectionid); ?>" title="<?php esc_attr_e( 'Export CSV', 'mjschool' ); ?>" type="submit" data-toggle="tooltip" class="mjschool-float-right mjschool-rootine-export-import-button mjschool-custom-padding-0"><img src="<?php echo esc_url( MJSCHOOL_PLUGIN_URL . "/assets/images/listpage-icon/mjschool-import-csv.png"); ?>"></a>
																	</div>
																</div>
															</button>
														</h4>
																			...
																			<td>
																				...
																				foreach ( $period as $period_data ) {
																					...
																					if ( get_option( 'mjschool_enable_virtual_classroom' ) === 'yes' ) {
																						...
																						if ( ! empty( $meeting_data ) ) {
																							$update_meeting      = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="admin.php?page=mjschool_virtual_classroom&tab=edit_meeting&action=edit&meeting_id=' . esc_attr( $meeting_data->meeting_id ) . '">' . esc_attr__( 'Edit Virtual Class', 'mjschool' ) . '</a></li>';
																							$delete_meeting      = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="admin.php?page=mjschool_virtual_classroom&tab=meeting_list&action=delete&meeting_id=' . $meeting_data->meeting_id . '" onclick="return confirm(\'' . esc_attr__( 'Are you sure you want to delete this record?', 'mjschool' ) . '\' );">' . esc_attr__( 'Delete Virtual Class', 'mjschool' ) . '</a></li>';
																							$meeting_statrt_link = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="' . $meeting_data->meeting_start_link . '" target="_blank">' . esc_attr__( 'Start Virtual Class', 'mjschool' ) . '</a></li>';
																						} else {
																							$update_meeting      = '';
																							$delete_meeting      = '';
																							$meeting_statrt_link = '';
																					...
																					if ( $school_type === 'university' ){
																						if ($cust_class_room === 1) {	
																							...
																							if( ! empty( $class_room ) )
																							{
																								echo '<span class="time"> ( ' . esc_attr($class_room->room_name) . ' ) </span>';
																							}
																						}
																					}
																					echo '<span class="time"> ( ' . esc_attr( $start_hour ) . ':' . esc_attr( $start_min ) . ' ' . esc_attr( $start_am_pm ) . ' - ' . esc_attr( $end_hour ) . ':' . esc_attr( $end_min ) . ' ' . esc_attr( $end_am_pm ) . ' ) </span>';
																					echo '</span><span class="caret"></span></button>';
																					echo '<ul class="pt-2 dropdown-menu mjschool-edit-delete-drop">
																							<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="?page=mjschool_route&tab=addroute&action=edit&route_id=' . esc_attr( $route_id ) . '&_wpnonce=' . esc_attr( mjschool_get_nonce( 'edit_action' ) ) . '">' . esc_attr__( 'Edit', 'mjschool' ) . '</a></li>
																							<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" onclick="return confirm(\'' . esc_attr__( 'Are you sure you want to delete this record?', 'mjschool' ) . '\' );" href="?page=mjschool_route&tab=route_list&action=delete&route_id=' . esc_attr( $route_id ) . '&_wpnonce=' . esc_attr( mjschool_get_nonce( 'delete_action' ) ) . '">' . esc_attr__( 'Delete', 'mjschool' ) . '</a></li>
						...
						<div class="mjschool-panel-white margin_top_20px"><!-------- Panel white. ------->
							<div class="mjschool-panel-body"><!-------- Panel body. ------->
								<div id="accordion" class="mjschool_fix_accordion panel-group accordion accordion-flush mjschool-padding-top-15px-res" aria-multiselectable="true" role="tablist">
									<?php $teacherdata = mjschool_get_users_data( 'teacher' );
																		...
																		if ( ! empty( $period ) ) {
																			...
																			foreach ( $period as $period_data ) {
																				...
																				if ( get_option( 'mjschool_enable_virtual_classroom' ) === 'yes' ) {
																					...
																					if ( ! empty( $meeting_data ) ) {
																						$update_meeting      = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="admin.php?page=mjschool_virtual_classroom&tab=edit_meeting&action=edit&meeting_id=' . $meeting_data->meeting_id . '">' . esc_attr__( 'Edit Virtual Class', 'mjschool' ) . '</a></li>';
																						$delete_meeting      = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="admin.php?page=mjschool_virtual_classroom&tab=meeting_list&action=delete&meeting_id=' . $meeting_data->meeting_id . '" onclick="return confirm(\'' . esc_attr__( 'Are you sure you want to delete this record?', 'mjschool' ) . '\' );">' . esc_attr__( 'Delete Virtual Class', 'mjschool' ) . '</a></li>';
																						$meeting_statrt_link = '<li class="mjschool-float-left-width-100px"><a class="mjschool-float-left-width-100px text-decoration-none" href="' . $meeting_data->meeting_start_link . '" target="_blank">' . esc_attr__( 'Virtual Class Start', 'mjschool' ) . '</a></li>';
																					} else {
																						$update_meeting      = '';
