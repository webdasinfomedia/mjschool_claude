 ...
 *
 * This file handles the addition, updating, and saving of student marks for multiple subjects.
 * It supports both university-type and school-type institutions, allowing teachers, support staff,
 * and administrators to manage exam results.
 *
 * @package MJSchool
			...
			<div class="row">
				...
				else
				{
					...
					<div class="col-md-3 input">
						...
						if ( isset( $_REQUEST['class_section'] ) ) {
							...
							$class_section = sanitize_text_field(wp_unslash($_REQUEST['section_id']));
						}
						?>
						...
						if ( isset( $_POST['exam_id'] ) ) {
							...
							if ( ! empty( $exam_data ) ) {
								foreach ( $exam_data as $retrieved_data ) {
									$posted_exam_id = isset( $_POST['exam_id'] ) ? sanitize_text_field( wp_unslash( $_POST['exam_id'] ) ) : '';
									?>
				...
				if ( $access === 1 ) {
					...
					<div class="form-group col-md-3">
						<input type="submit" value="<?php esc_html_e( 'Go', 'mjschool' ); ?>" name="add_multiple_subject_marks" class="btn height-auto btn-info mjschool-save-btn" />
					</div>
					<?php
				}
				?>
...
if ( isset( $_REQUEST['add_single_student_mark'] ) ) {
	if ( $school_type === "university" ) {
		$keys = array_keys( wp_unslash( $_REQUEST['add_single_student_mark'] ) );
		if ( ! empty( $keys[0] ) ) {
			list( $user_id, $subject_id ) = explode( '_', $keys[0] );
			$user_id    = (int) $user_id;
			...
			$marks   = isset( $_REQUEST[ $mark_key ] ) ? (int) sanitize_text_field( wp_unslash( $_REQUEST[ $mark_key ] ) ) : 0;
			$mark_id_val = isset( $_REQUEST[ $mark_id_key ] ) ? (int) sanitize_text_field( wp_unslash( $_REQUEST[ $mark_id_key ] ) ) : 0;
			...
			if ( $contributions === 'yes' ) {
				...
				// Ensure it's an array and sanitize each element
			...
			if ( $result ) {
				...
				if ( is_super_admin() ) {
					if ( $flag === 1 ) {
						wp_redirect( admin_url() . 'admin.php?page=mjschool_result&tab=multiple_subject_marks&_wpnonce=' . esc_attr( $nonce ) . '&message=3' );
					} elseif ( $flag === 0 ) {
						wp_redirect( admin_url() . 'admin.php?page=mjschool_result&tab=multiple_subject_marks&_wpnonce=' . esc_attr( $nonce ) . '&message=4' );
					}
				} else {
					wp_redirect( home_url() . '?dashboard=mjschool_user&page=manage-marks&tab=multiple_subject_marks&_wpnonce=' . esc_attr( $nonce ) . '&message=4' );
				}
				exit;
			}
		}
		...
		// Non-university flow
		$user_id      = (int) sanitize_text_field( wp_unslash( $_REQUEST['add_single_student_mark'] ) );
		$section_id   = isset( $_REQUEST['section_id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['section_id'] ) ) : '';
		$class_id     = isset( $_REQUEST['class_id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['class_id'] ) ) : '';
		$exam_id      = isset( $_REQUEST['exam_id'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['exam_id'] ) ) : '';
		...
		foreach ( $subject_list as $sub_id ) {
			...
			if ( $contrib_value === 'yes' ) {
				...
				$marks = isset( $_REQUEST[ $marks_key ] ) ? (int) sanitize_text_field( wp_unslash( $_REQUEST[ $marks_key ] ) ) : 0;
				$class_marks = '';
			}
			$grade_id = $mjschool_obj_marks->mjschool_get_grade_id( $marks );
			if ( ! $grade_id ) {
			...
			if ( $mark_detail ) {
				$mark_id_raw = isset( $_REQUEST[ $markid_key ] ) ? wp_unslash( $_REQUEST[ $markid_key ] ) : 0;
				$mark_id     = array( 'mark_id' => (int) sanitize_text_field( $mark_id_raw ) );
...
if ( isset( $_POST['save_all_multiple_subject_marks'] ) ) {
	$subject_list = $mjschool_obj_marks->mjschool_student_subject( $class_id, sanitize_text_field(wp_unslash($_REQUEST['section_id'])) );
	...
	if ( isset( $_REQUEST['section_id']) && $_REQUEST['section_id'] != "") {
		...
		$student = get_users(array(
			...
			'meta_value' => sanitize_text_field(wp_unslash($_REQUEST['section_id'])),
	...
	foreach ( $student as $mjschool_user ) {
		foreach ( $subject_list as $sub_id ) {
			$mark_detail = $mjschool_obj_marks->mjschool_subject_makrs_detail_byuser( sanitize_text_field(wp_unslash($_REQUEST['exam_id'])), $class_id, $sub_id->subid, $mjschool_user->ID );
			$comment     = sanitize_text_field(wp_unslash($_REQUEST[ 'marks_' . $mjschool_user->ID . '_' . $sub_id->subid . '_comment' ]));
			if ( isset($_POST['contributions']) && sanitize_text_field(wp_unslash($_POST['contributions'])) === 'yes' ) {
				$class_marks = json_encode( $_REQUEST[ 'class_marks_' . $mjschool_user->ID . '_' . $sub_id->subid . '_mark' ] );
				$marks       = 0;
			} else {
				$marks       = sanitize_text_field(wp_unslash($_REQUEST[ 'marks_' . $mjschool_user->ID . '_' . $sub_id->subid . '_mark' ]));
				$class_marks = '';
			}
			$grade_id = $mjschool_obj_marks->mjschool_get_grade_id( $marks );
			if ( ! $grade_id ) {
			...
			if ( $mark_detail ) {
				$mark_id = sanitize_text_field(wp_unslash($_REQUEST[ 'marks_' . $mjschool_user->ID . '_' . $sub_id->subid . '_mark_id' ]));
...
if ( isset( $_POST['add_multiple_subject_marks'] ) || isset( $_POST['add_single_student_mark'] ) || isset( $_POST['save_all_multiple_subject_marks'] ) ) {
	...
	if ( isset( $_REQUEST['class_section']) && $_REQUEST['class_section'] != "") {
		$subject_list = $mjschool_obj_marks->mjschool_student_subject_for_list($class_id, sanitize_text_field(wp_unslash($_REQUEST['class_section'])));
		$exlude_id = mjschool_approve_student_list();
			...
			<div class="clearfix panel-body p table_overflow_scroll">
				<form method="post" class="form-inline add_multiple_subject_mark_form" id="marks_form" enctype="multipart/form-data">
					...
					<div class="table-responsive">
						<?php
						// STEP 1: Fetch all subject marks data for the exam once.
						$exam_data=$mjschool_exam_obj->mjschool_exam_data(sanitize_text_field(wp_unslash($_POST['exam_id'])));
						$all_subjects_max_marks_data = []; // Default to an empty array.
						...
						// ========= START: Main loop for each subject. =========
						if ( ! empty( $subject_list ) ) {
							foreach ($subject_list as $sub_id) {
								// STEP 2: Find the max marks for the current subject in the loop.
								$current_subject_max_marks = null; // Set a default value.
								...
								<table class="table table-bordered col-md-12">
									<thead>
										<tr>
											<th><?php esc_attr_e( 'Roll No.', 'mjschool' ); ?></th>
											<th><?php esc_attr_e( 'Name', 'mjschool' ); ?></th>
											<th><?php esc_attr_e( 'Mark( '.$current_subject_max_marks.' )', 'mjschool' ); //phpcs:ignore ?></th>
											<th><?php esc_attr_e( 'Comment', 'mjschool' ); ?></th>
											<th><?php esc_attr_e( 'Action', 'mjschool' ); ?></th>
										</tr>
									</thead>
									<tbody>
										<?php
										// Loop through all students to find those enrolled in this subject.
										foreach ($student as $mjschool_user) {
											...
											// Initialize variables for this student.
						...
						} // End if !empty mjschool-subject-list.
						?>
					</div>
					<?php
		...
		else
		{
			...
			<div class="clearfix mjschool-panel-body p table_overflow_scroll">
				<form method="post" class="form-inline add_multiple_subject_mark_form" id="marks_form" enctype="multipart/form-data">
					...
					<input type="hidden" name="class_id" value="<?php echo esc_attr( $class_id ); ?>" />
					<input type="hidden" name="section_id" value="<?php if( isset( $_REQUEST['class_section'] ) ) {echo esc_attr( sanitize_text_field(wp_unslash($_REQUEST['class_section'])) );} ?>" />
					<input type="hidden" name="" value="<?php if( isset( $_REQUEST['class_section'] ) ) {echo esc_attr( sanitize_text_field(wp_unslash($_REQUEST['class_section'])) );} ?>" />
					<input type="hidden" name="contributions" value="<?php echo esc_attr( $contributions ); ?>" />
					<div class="table-responsive">
					...
					if ( $access === 1 ) {
						...
						<div class="col-sm-6 mjschool-margin-top-20px mjschool-padding-top-25px-res">
							<input type="submit" class="btn btn-success mjschool-save-btn" name="save_all_multiple_subject_marks" value="<?php esc_html_e( 'Update All Marks', 'mjschool' ); ?>">
						</div>
						<?php
					}
					?>
