...
class Mjschool_Hostel
{
    ...
    public function mjschool_insert_hostel( $data )
    {
        ...
        $hostel_data['hostel_name'] = stripslashes(sanitize_text_field($data['hostel_name']));
        $hostel_data['hostel_type'] = stripslashes(sanitize_text_field($data['hostel_type']));
        $hostel_data['hostel_address'] = stripslashes(sanitize_text_field($data['hostel_address']));
        $hostel_data['hostel_intake']  = stripslashes($data['hostel_intake']);
        $hostel_data['Description'] = stripslashes(sanitize_textarea_field($data['Description']));
        if ($data['action'] == 'edit' ) {
            ...
            mjschool_append_audit_log('' . esc_html__('Hostel Updated', 'mjschool') . '( ' . $hostel . ' )' . '', get_current_user_id(), get_current_user_id(), 'edit', sanitize_text_field(wp_unslash($_REQUEST['page'])));
            return $result;
        } else {
            ...
            $ids    = $wpdb->insert_id;
            $hostel = $hostel_data['hostel_name'];
            mjschool_append_audit_log('' . esc_html__('Hostel Added', 'mjschool') . '( ' . $hostel . ' )' . '', get_current_user_id(), get_current_user_id(), 'insert', sanitize_text_field(wp_unslash($_REQUEST['page'])));
            return $ids;
        }
    }
    /**
    ...
    public function mjschool_delete_hostel( $id )
    {
        ...
        mjschool_append_audit_log('' . esc_html__('Hostel Deleted', 'mjschool') . '( ' . $hostel_name . ' )' . '', get_current_user_id(), get_current_user_id(), 'delete', sanitize_text_field(wp_unslash($_REQUEST['page'])));
     	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
        return $result = $wpdb->query($wpdb->prepare("DELETE FROM $table_mjschool_hostel WHERE id= %d", $hostel_id));
    }
    /**
    ...
    public function mjschool_get_all_hostel()
    {
        ...
        $table_mjschool_hostel = $wpdb->prefix . 'mjschool_hostel';
     	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
        $result = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_mjschool_hostel"));
        return $result;
    }
    /**
    ...
    {
        if (! empty($data['mjschool_hostel_room_facilities']) ) {
            $facilities = json_encode($data['mjschool_hostel_room_facilities']);
        } else {
            $facilities = '';
        }
        global $wpdb;
        $table_mjschool_room               = $wpdb->prefix . 'mjschool_room';
        $room_data['room_unique_id']   = sanitize_text_field($data['room_unique_id']);
        $room_data['hostel_id']        = sanitize_text_field($data['hostel_id']);
        $room_data['room_status']      = '0';
        $room_data['room_category']    = $data['room_category'];
        $room_data['beds_capacity']    = $data['beds_capacity'];
        $room_data['room_description'] = sanitize_textarea_field(stripslashes($data['room_description']));
        ...
        if ($data['action'] == 'edit_room' ) {
            ...
            mjschool_append_audit_log('' . esc_html__('Room Updated', 'mjschool') . '( ' . $room . ' )' . '', get_current_user_id(), get_current_user_id(), 'edit', sanitize_text_field(wp_unslash($_REQUEST['page'])));
            return $result;
        } else {
            ...
            $room   = $room_data['room_unique_id'];
            mjschool_append_audit_log('' . esc_html__('Room Added', 'mjschool') . '( ' . $room . ' )' . '', get_current_user_id(), get_current_user_id(), 'insert', sanitize_text_field(wp_unslash($_REQUEST['page'])));
            return $result;
        }
    }
    /**
    ...
    public function mjschool_delete_room( $id )
    {
        ...
        $room = $room_data->room_unique_id;
        mjschool_append_audit_log('' . esc_html__('Room Deleted', 'mjschool') . '( ' . $room . ' )' . '', get_current_user_id(), get_current_user_id(), 'delete', sanitize_text_field(wp_unslash($_REQUEST['page'])));
     	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
        return $result = $wpdb->query($wpdb->prepare("DELETE FROM $table_mjschool_room WHERE id= %d", $room_id));
    }
    /**
    ...
    public function mjschool_insert_bed( $data )
    {
        ...
        $bed_data['bed_unique_id'] = sanitize_text_field($data['bed_unique_id']);
        $bed_data['room_id']       = $data['room_id'];
        $bed_data['bed_charge'] = $data['bed_charge'];
        $bed_data['bed_description'] = sanitize_textarea_field(stripslashes($data['bed_description']));
        if ($data['action'] == 'edit_bed' ) {
            $bed_data['updated_by']   = get_current_user_id();
            $bed_data['updated_date'] = date('Y-m-d');
            $bed_id['id']             = sanitize_text_field(intval($data['bed_id']));
         	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
            $result = $wpdb->update($table_mjschool_beds, $bed_data, $bed_id);
            $bed = $bed_data['bed_unique_id'];
            mjschool_append_audit_log('' . esc_html__('Bed Updated', 'mjschool') . '( ' . $bed . ' )' . '', get_current_user_id(), get_current_user_id(), 'edit', sanitize_text_field(wp_unslash($_REQUEST['page'])));
            return $result;
        } else {
            ...
            $bed    = $bed_data['bed_unique_id'];
            mjschool_append_audit_log('' . esc_html__('Bed Added', 'mjschool') . '( ' . $bed . ' )' . '', get_current_user_id(), get_current_user_id(), 'insert', sanitize_text_field(wp_unslash($_REQUEST['page'])));
            return $result;
        }
    }
    /**
    ...
    public function mjschool_delete_bed( $id )
    {
        ...
        mjschool_append_audit_log('' . esc_html__('Bed Deleted', 'mjschool') . '( ' . $bed . ' )' . '', get_current_user_id(), get_current_user_id(), 'delete', sanitize_text_field(wp_unslash($_REQUEST['page'])));
     	// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
        return $result = $wpdb->query($wpdb->prepare("DELETE FROM $table_mjschool_beds WHERE id= %d", $bed_id));
    }
    /**
    ...
    public function mjschool_assign_room( $data )
    {
        ...
        $table_mjschool_beds        = $wpdb->prefix . 'mjschool_beds';
        $table_mjschool_assign_beds = $wpdb->prefix . 'mjschool_assign_beds';
        $hostel_id              = intval(mjschool_decrypt_id($data['hostel_id']));
            ...
            foreach ( $data['room_id_new'] as $key => $value ) {
                ...
                if (! empty($student_unique) ) {
                    ...
                    if (! empty($assign_bed_data) ) {
                        ...
                        mjschool_append_audit_log('' . esc_html__('Assign Room Updated', 'mjschool') . '', get_current_user_id(), get_current_user_id(), 'edit', sanitize_text_field(wp_unslash($_REQUEST['page'])));
                        if ($result ) {
                        ...
                        mjschool_append_audit_log('' . esc_html__('Assign Bed Added', 'mjschool') . '', get_current_user_id(), get_current_user_id(), 'insert', sanitize_text_field(wp_unslash($_REQUEST['page'])));
                        if ($result ) {
                               ...
                               $string                     = array();
                               $string['{{student_name}}'] = mjschool_get_display_name($student_unique);
                               $string['{{hostel_name}}']  = mjschool_hostel_name_by_id($data['hostel_id']);
                               $string['{{room_id}}']      = mjschool_get_room_unique_id_by_room_id($value);
                               ...
                               $title             = esc_attr__('New Notification For Assign Bed.', 'mjschool');
                               $text              = esc_attr__('You have been assigned new Bed', 'mjschool') . ' ' . $data['bed_unique_id'][ $key ];
    ...
    {
        mjschool_append_audit_log('' . esc_html__('Assign Bed Deleted', 'mjschool') . '', get_current_user_id(), get_current_user_id(), 'delete', sanitize_text_field(wp_unslash($_REQUEST['page'])));
        global $wpdb;
        $room_id                = intval($rid);
        $bed_id                 = intval($bid);
    ...
    public function mjschool_get_bed_by_hostel_id( $id )
    {
        ...
        if (! empty($room_data) ) {
            foreach ( $room_data as $value ) {
                $bed_data = array_merge($bed_data, $this->mjschool_get_all_bed_by_room_id($value->id));
            }
        }
        return $bed_data;
    }
