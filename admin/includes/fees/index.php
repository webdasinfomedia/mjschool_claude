...
if ( isset( $_POST['save_feetype'] ) ) {
	...
	if ( isset( $_GET['action'] ) && sanitize_text_field( wp_unslash( $_GET['action'] ) ) === 'edit' ) {
		if ( isset( $_GET['_wpnonce_action'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce_action'] ) ), 'edit_action' ) ) {
			...
			$result                    = $obj_lib->mjschool_add_book( array_map( 'sanitize_text_field', wp_unslash( $_POST ) ) );
			$mjschool_custom_field_obj = new Mjschool_Custom_Field();
			$module                = 'fee_pay';
	...
	} elseif ( isset( $_POST['fees_title_id'] ) && isset( $_POST['class_id'] ) && ! $mjschool_obj_fees->mjschool_is_duplicat_fees( sanitize_text_field( wp_unslash( $_POST['fees_title_id'] ) ), sanitize_text_field( wp_unslash( $_POST['class_id'] ) ) ) ) {

		$result                    = $mjschool_obj_fees->mjschool_add_fees( array_map( 'sanitize_text_field', wp_unslash( $_POST ) ) );
		$mjschool_custom_field_obj = new Mjschool_Custom_Field();
		$module                = 'fee_pay';
		...
		<div class="panel mjschool-panel-white">
			<div class="mjschool-panel-body">     
				<h2 class="nav-tab-wrapper">
					...
					<?php
					// Use $_GET specifically instead of $_REQUEST for security.
					if ( isset( $_GET['action'] ) && sanitize_text_field( wp_unslash( $_GET['action'] ) ) === 'edit' && isset( $_GET['tab'] ) && sanitize_text_field( wp_unslash( $_GET['tab'] ) ) === 'addfeetype' ) {
						?>
						<a href="<?php echo esc_url( admin_url( 'admin.php?page=mjschool_fees&tab=addfeetype&action=edit&fees_id=' . rawurlencode( sanitize_text_field( wp_unslash( $_GET['fees_id'] ) ) ) ) ); ?>" class="nav-tab <?php echo esc_attr( $active_tab ) === 'addfeetype' ? 'nav-tab-active' : ''; ?>">
								...
								<tbody>
									...
									foreach ( $retrieve_class_data as $retrieved_data ) {
										...
										<tr>
											...
											<td><?php $mjschool_class = new Mjschool_Class(); echo esc_html( $mjschool_class->mjschool_get_class_name( $retrieved_data->class_id ) ); ?></td>
