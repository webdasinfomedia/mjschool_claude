...
switch ( $mjschool_action ) {
	case 'process': // case process insert the form data in DB and process to the paypal
		...
		$p->add_field( 'business', MJSCHOOL_PAYPAL_EMAIL_ADD ); // Call the facilitator eaccount
		$p->add_field( 'cmd', $_POST['cmd'] ); // cmd should be _cart for cart checkout
		$p->add_field( 'upload', '1' );
		$p->add_field( 'return', home_url() . '/?dashboard=mjschool_user&page=feepayment?action=success' ); // return URL after the transaction got over
		$p->add_field( 'cancel_return', home_url() . '/?dashboard=mjschool_user&page=feepayment?action=cancel' ); // cancel URL if the trasaction was cancelled during half of the transaction
	...
	case 'success': // success case to show the user payment got success
		...
		$url = 'http://' . sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) . sanitize_text_field( wp_unslash( $_SERVER['PHP_SELF'] ) ) . '?action=ipn';
		echo '<h3>' . esc_url( $url ) . '</h3>';
		echo '</div>';
		break;
