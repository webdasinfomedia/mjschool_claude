...
class Mjschool_Hostel {
	...
	public function mjschool_insert_hostel( $data ) {
		...
		$hostel_data['hostel_name']     = isset( $data['hostel_name'] ) ? stripslashes( sanitize_text_field( $data['hostel_name'] ) ) : '';
		$hostel_data['hostel_type']     = isset( $data['hostel_type'] ) ? stripslashes( sanitize_text_field( $data['hostel_type'] ) ) : '';
		$hostel_data['hostel_address']  = isset( $data['hostel_address'] ) ? stripslashes( sanitize_text_field( $data['hostel_address'] ) ) : '';
		$hostel_data['hostel_intake']   = isset( $data['hostel_intake'] ) ? intval( $data['hostel_intake'] ) : 0;
		$hostel_data['Description']     = isset( $data['Description'] ) ? stripslashes( sanitize_textarea_field( $data['Description'] ) ) : '';
		...
		if ( $action === 'edit' ) {
			...
			$hostel_data['updated_date'] = gmdate( 'Y-m-d' );
			...
			$hostel_data['created_date'] = gmdate( 'Y-m-d' );
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
	...
	public function mjschool_insert_room( $data ) {
		if ( ! empty( $data['mjschool_hostel_room_facilities'] ) && is_array( $data['mjschool_hostel_room_facilities'] ) ) {
			$facilities_sanitized = array_map( 'sanitize_text_field', $data['mjschool_hostel_room_facilities'] );
		...
		$room_data['room_unique_id']   = isset( $data['room_unique_id'] ) ? sanitize_text_field( $data['room_unique_id'] ) : '';
		...
		$room_data['room_description'] = isset( $data['room_description'] ) ? sanitize_textarea_field( stripslashes( $data['room_description'] ) ) : '';
		$room_data['facilities']       = $facilities;
		$page_name                     = isset( $_REQUEST['page'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['page'] ) ) : '';
		$action                        = isset( $data['action'] ) ? sanitize_text_field( $data['action'] ) : '';
		if ( $action === 'edit_room' ) {
			$room_data['updated_by']   = get_current_user_id();
			$room_data['updated_date'] = gmdate( 'Y-m-d' );
			...
			$room_data['created_date'] = gmdate( 'Y-m-d' );
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
	...
	public function mjschool_insert_bed( $data ) {
		...
		$bed_data['bed_unique_id']   = isset( $data['bed_unique_id'] ) ? sanitize_text_field( $data['bed_unique_id'] ) : '';
		...
		$bed_data['bed_description'] = isset( $data['bed_description'] ) ? sanitize_textarea_field( stripslashes( $data['bed_description'] ) ) : '';
		$page_name                   = isset( $_REQUEST['page'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['page'] ) ) : '';
		$action                      = isset( $data['action'] ) ? sanitize_text_field( $data['action'] ) : '';
		if ( $action === 'edit_bed' ) {
			$bed_data['updated_by']   = get_current_user_id();
			$bed_data['updated_date'] = gmdate( 'Y-m-d' );
			...
			$bed_data['created_date'] = gmdate( 'Y-m-d' );
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Safe direct query, caching not required in this context
	...
	public function mjschool_assign_room( $data ) {
		...
		$table_mjschool_beds        = $wpdb->prefix . 'mjschool_beds';
		$table_mjschool_assign_beds = $wpdb->prefix . 'mjschool_assign_beds';
		$hostel_id_raw              = isset( $data['hostel_id'] ) ? sanitize_text_field( $data['hostel_id'] ) : '';
		...
		if ( ! empty( $data['room_id_new'] ) && is_array( $data['room_id_new'] ) ) {
			foreach ( $data['room_id_new'] as $key => $value ) {
				...
				if ( ! empty( $student_unique ) ) {
					...
					$bed_unique_id   = isset( $data['bed_unique_id'][ $key ] ) ? sanitize_text_field( $data['bed_unique_id'][ $key ] ) : '';
					$assign_date_raw = isset( $data['assign_date'][ $key ] ) ? sanitize_text_field( $data['assign_date'][ $key ] ) : gmdate( 'Y-m-d' );
					if ( ! empty( $assign_bed_data ) ) {
						...
						$assign_data['assign_date']   = gmdate( 'Y-m-d', strtotime( $assign_date_raw ) );
						$assign_data['created_date']  = gmdate( 'Y-m-d' );
						...
						if ( $result ) {
							...
							$string['{{hostel_name}}']  = mjschool_hostel_name_by_id( $hostel_id_raw );
							$string['{{room_id}}']      = mjschool_get_room_unique_id_by_room_id( intval( $value ) );
							$string['{{bed_id}}']       = $bed_unique_id;
							$string['{{bed_charge}}']   = html_entity_decode( $currency_symbol ) . '' . floatval( $bed_data->bed_charge );
