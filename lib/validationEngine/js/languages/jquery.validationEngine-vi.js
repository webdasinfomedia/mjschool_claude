	...
	$.validationEngineLanguage    = {
		newLang: function () {
			$.validationEngineLanguage.allRules = {
				...
				"onlyNumberSp": {
					"regex": /^[0-9 ]+$/,
					"alertText": "* Chỉ được nhập số và khoảng trắng"
				},

				"onlyLetterSp": {
					"regex": /^[\p{L} ']+$/u,
					"alertText": "* Chỉ được nhập chữ cái, khoảng trắng và dấu nháy đơn ( ' )"
				},
				"onlyLetterAccentSp": {
					"regex": /^[\p{L} ]+$/u,
					"alertText": "* Chỉ được nhập chữ cái và khoảng trắng"
				},
				"onlyLetterNumber": {
					"regex": /^[\p{L}0-9]+$/u,
					"alertText": "* Không được chứa ký tự đặc biệt, chỉ chấp nhận chữ cái và số"
				},

				// Start Custom Validation
				// 1)First Name,Last Name
				"onlyLetter_specialcharacter": {
					"regex": /^[A-Za-zÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúýĂăĐđĨĩŨũƯưẠ-ỹ\s_,`.'^-]+$/,
					"alertText": "* Chỉ có chữ cái và các ký tự ' _,`.'^-' được phép"
				},
				...
				"popup_category_validation": {
					"regex": /^[\p{L}0-9 _\-,`.'^]+$/u,
					"alertText": "* Chỉ có chữ cái, số và các ký tự ' _,`.'^-' được phép"
				},
				...
				"phone_number": {
					"regex": /^[0-9 \-+()]+$/,
					"alertText": "* Chỉ số và các ký tự ' -+()' được phép"
				},
